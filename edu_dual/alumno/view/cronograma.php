...
if (isset($_REQUEST['idinscripcion'])) {
    ...
    $stmt->execute();
    //tutores_academicos ON inscripciones.idpersonal = tutores_academicos.idtutor_academico
    $resultados = $stmt->fetchAll(PDO::FETCH_ASSOC);

    foreach ($resultados as $fila) {
        echo "<h2><center><b>Detalle del calendario de actividades</b></center></h2> <br>";
        echo "&nbsp;&nbsp;Alumno: <b>" . htmlspecialchars($fila['nombre']) . " " . htmlspecialchars($fila['apellidop']) . " " . htmlspecialchars($fila['apellidom']) . "</b><br>";
...
function getColor($descripcion) {
    return $descripcion ? '#A1FF9A' : '#FF9999'; //A1FF9A
	//return $descripcion ? '#FF9999' : '#FFFFFF';
}
?>